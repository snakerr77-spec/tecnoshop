...
function Hero() {
  ...
  return <section className="hero" id="inicio" onMouseMove={move} onMouseLeave={() => { x.set(0); y.set(0); }}>
    ...
    <div className="container hero-grid">
      ...
      <motion.div className="hero-visual" style={{ x, y }} initial={{ opacity: 0, x: 70 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: .15, duration: 1, ease: [.16, 1, .3, 1] }}>
        <div className="orbit orbit-a"><i /></div><div className="orbit orbit-b"><i /></div>
        <img className="hero-products" src="./assets/hero-products.png" alt="Smartphone, notebook, smartwatch, fones e scooter elétrica" />
        <motion.div className="hero-brand-seal" animate={{ y: [0, -9, 0] }} transition={{ duration: 4, repeat: Infinity }}><img src="./assets/tecnoshop-logo.png" alt="" /></motion.div>
        <motion.div className="finance-pill" animate={{ y: [0, -7, 0] }} transition={{ duration: 3.5, repeat: Infinity }}><CreditCard size={18} /><span><b>Até 18x</b><small>no cartão</small></span></motion.div>
        <motion.div className="price-card" animate={{ y: [0, 9, 0] }} transition={{ duration: 4.4, repeat: Infinity }}><small>SEMINOVO PREMIUM</small><strong>iPhone 15 Pro Max</strong><span>a partir de R$ 4.200*</span></motion.div>
      </motion.div>
    </div>
    <div className="container footnote">*Preço divulgado publicamente pela loja. Confirme estoque, condições e valor atualizado pelo WhatsApp.</div>
  </section>;
}
...
function Trade() {
  return <section className="trade" id="troca"><div className="container trade-grid">
    <motion.div className="trade-art" initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: .8 }}><div className="trade-rings" /><motion.div className="old-phone" animate={{ y: [15, 2, 15], rotate: [-7, -5, -7] }} transition={{ duration: 5, repeat: Infinity }}><i /><span>SEU</span><strong>IPHONE</strong><small>ATUAL</small></motion.div><div className="trade-arrow"><ArrowRight /></div><motion.div className="new-phone" animate={{ y: [-15, -28, -15], rotate: [7, 5, 7] }} transition={{ duration: 5, repeat: Infinity }}><i /><span>NOVO</span><strong>UPGRADE</strong><small>TECNOSHOP</small></motion.div></motion.div>
